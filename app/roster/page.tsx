  ...
  return (
    <PageContainer>
      ...
      {!hasActiveCoach && currentTeammate && (
        <div className="mt-6 flex flex-col gap-3 rounded-lg border border-brand-dim bg-brand-dim/20 p-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          ...
          <ActionForm
            ...
            className="tap-target shrink-0 self-start font-mono text-[11px] font-semibold uppercase tracking-wide text-brand-bright transition-colors duration-[var(--d-micro)] hover:text-brand disabled:opacity-60"
          >
            Become Coach
          </ActionForm>
        </div>
        ...
        <div className="mt-8">
          ...
          <div className="mt-2 flex flex-col gap-2">
            {inactive.map((t) => (
              <div
                key={t.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-surface/50 p-4 opacity-70"
              >
                <div className="flex min-w-0 items-center gap-3">
                  <Avatar name={t.user.name ?? "?"} src={t.user.image} size={28} />
                  <p className="min-w-0 truncate text-body text-text-muted">{t.user.name}</p>
                </div>
                <ActionForm
